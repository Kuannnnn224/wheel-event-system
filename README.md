...
若要讓後端產生的 webview URL 指到 GitHub Pages，`backend-node8/.env` 至少需要：
...
`WEBVIEW_API_BASE_URL` 使用絕對 HTTP(S) URL 時，後端產生的 `webviewUrl` 會自動帶上 `apiBase` query，讓 GitHub Pages 上的靜態 webview 可以呼叫正確 API。若 webview 與 API 同源部署，維持 `/api` 即可。
...
- `admin_users`：後控帳號。
- `players`：玩家主檔，`id` 直接使用平台帶入的玩家 ID，不再區分內部/外部 ID。
- `player_daily_progress`：玩家每日流水與解鎖階段。
- `spin_records`：真實抽獎紀錄。
- `award_override_rules`：後控指定派獎規則。
- `webview_sessions`：app/webview 存取 session，保存 webview 啟動用 token 與到期時間。

舊版 `turnover_adjustments` 不再屬於 Node 8 runtime baseline；若既有測試 DB 還留著，可在確認不需要保留歷史資料後移除。
若既有 DB 還有 `players.external_id`，本分支新的 baseline 已移除此欄位；本機測試環境建議直接重建 DB，正式資料需先規劃 migration，把平台玩家 ID 寫回 `players.id` 與所有關聯表 `player_id`。

若現有 DB 的 table comment 還是舊的 demo 命名，可用這句非破壞性更新：
